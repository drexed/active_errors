...
RSpec.describe ActiveErrors::Messages do
  subject { ActiveErrors::Messages.new }

  let(:user) { User.create(name: 'Lebron James') }
  ...
  describe '.merge!' do
    it 'returns { field: ["error message"], name: ["can\'t be blank"]}' do
      user.update(name: nil)

      subject.add(:field, 'error message')
      subject.merge!(user.errors)

      expect(subject.errors).to eq({ field: ['error message'], name: ["can't be blank"]})
    end
